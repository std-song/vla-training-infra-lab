...
def main() -> None:
    ...
    rows = read_csv(RESULTS / "qwen25vl_bucketed_prefix_cache_sim.csv")
    rows.sort(key=lambda r: (r["policy"], int(r["prefix_cache"])))
    labels = [f'{r["policy"].replace("_bucket", "")}\\ncache={r["prefix_cache"]}' for r in rows]
    bar_svg(
        "Bucketed Scheduler Throughput",
        labels,
        [float(r["throughput_req_s"]) for r in rows],
        "requests/s",
        FIGURES / "project3_bucketed_scheduler_throughput.svg",
    )
    bar_svg(
        "Bucketed Scheduler P95 Latency",
        labels,
        [float(r["p95_latency_ms"]) for r in rows],
        "P95 latency ms",
        FIGURES / "project3_bucketed_scheduler_p95.svg",
    )
    bar_svg(
        "Visual Token Padding Waste",
        labels,
        [float(r["padding_waste_ratio"]) * 100 for r in rows],
        "padding waste %",
        FIGURES / "project3_bucketed_scheduler_padding_waste.svg",
    )
